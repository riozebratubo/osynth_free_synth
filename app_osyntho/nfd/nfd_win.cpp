...
/* Sets the folder the dialog opens in and, when defaultPath names a file rather
   than an existing directory, the file name it opens pre-filled with.

   Upstream only ever called SetFolder(), so a save dialog could not suggest a
   name -- the Qt FileDialog this replaces did that through selectedFile, and
   dropping it would mean retyping the patch name on every export. The zenity
   backend already accepts a full path in --filename, so this brings Windows in
   line with it rather than introducing a platform quirk. */
static nfdresult_t SetDefaultPath( IFileDialog *dialog, const char *defaultPath )
{
    ...
    /* An existing directory is used whole; anything else is split at its last
       separator. Both separators are accepted, since a caller may hand over a
       path that never went through QDir::toNativeSeparators(). */
    const DWORD attribs = ::GetFileAttributesW( defaultPathW );
    if ( attribs == INVALID_FILE_ATTRIBUTES || !(attribs & FILE_ATTRIBUTE_DIRECTORY) )
    {
        wchar_t *sep = wcsrchr( defaultPathW, L'\\' );
        wchar_t *fwd = wcsrchr( defaultPathW, L'/' );
        if ( !sep || ( fwd && fwd > sep ) )
            sep = fwd;

        /* sep == defaultPathW would leave the folder an empty string ("\name"),
           and a trailing separator leaves no name to set. */
        if ( sep && sep > defaultPathW && *(sep+1) != L'\0' )
        {
            dialog->SetFileName( sep + 1 );   /* copies -- safe to truncate after */
            *sep = L'\0';
        }
    }
