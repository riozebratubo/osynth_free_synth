...
{    
    ...
    if(stdOut != NULL)
    {
        ...
        // A cancelled dialog prints nothing, but runCommandArray() still hands
        // back an allocated empty string rather than NULL -- and the len-1 write
        // below would then land one byte BEFORE the allocation. Answer NULL for
        // that case instead, which is what the caller already expects.
        if(len > 0)
        {
                                    ...
                                    nfdpathset_t *outPaths )
{
    ...
    {
        size_t len = strlen(stdOut);
        // Same guard as the single-selection dialogs: a cancelled pick yields an
        // allocated empty string, and stdOut[len-1] would write out of bounds.
        if(len > 0)
        {
