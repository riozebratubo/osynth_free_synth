...
void SynthController::resetState() {
  ...
  // The chord set belongs to whichever synth reported it, and so does the
  // answer to "does this firmware have chord mode at all". Carrying either
  // across a reconnect would let the editor show — and then write back — a
  // set the connected instrument never sent.
  m_chordSet.clear();
  m_chordAvailable = false;
...
void SynthController::finishDiscovery() {
  ...
  // page stays hidden.
  refreshGraph();
  // And the user chord set (S41), for the third time the same reason: twelve
  // slots of intervals are not parameter space. Doubles as the capability
  // probe — firmware that predates chord mode refuses the opcode, so
  // chordAvailable stays false and the page hides its user-set editor.
  refreshChordSet();
  m_songAccum.clear();
...
TrackConfig SynthController::trackConfig() const {
  ...
  c.chord = (m_trackCfg.flags & SEQ_TRACK_F_CHORD) != 0;
...
void SynthController::setTrackField(const QString& field, double value) {
  ...
  } else if (field == QLatin1String("chord")) {
    // Only this bit. Mute and solo live in the same byte but belong to the
    // trk<N>.mute/solo parameters — the firmware puts its own copy back over
    // whatever we send, so touching them here would be a write that silently
    // does nothing.
    c.flags = quint8(v ? (c.flags | SEQ_TRACK_F_CHORD)
                       : (c.flags & ~SEQ_TRACK_F_CHORD));
  } else {
    return;
  }
...
QString SynthController::chordNameFor(int note) const {
  ...
  if (q >= 0) {
    return QString::fromLatin1(kPitchNames[((root % 12) + 12) % 12]) +
           QString::fromLatin1(kQuality[q].name);
  }
  ...
  for (int n : notes)
    spelled << QString::fromLatin1(kPitchNames[((n % 12) + 12) % 12]);
  return spelled.join(QLatin1Char(' '));
}

QList<int> SynthController::chordDegreeKeys(int octaves) const {
  QList<int> keys;
  int scale = 0, root = 0;
  chordScaleRoot(&scale, &root);
  int semi[12];
  ...
  const int base = scaleBase(root);
  const int reps = qBound(1, octaves, 4);
  keys.reserve(n * reps);
  for (int o = 0; o < reps; ++o) {
    for (int i = 0; i < n; ++i) {
      // A degree keymap advances one *semitone* per degree, so its second
      // octave starts n keys up, not twelve. That is the whole difference
      // between the two mappings and the reason this is computed here rather
      // than by the page adding 12.
      const int k = degrees ? kDegreeOrigin + o * n + i : base + 12 * o + semi[i];
      if (k >= 0 && k <= 127) keys.append(k);
    }
  }
  return keys;
}
...
QStringList SynthController::chordQualityNames() const {
  ...
  for (int i = 0; i < kQualityCount; ++i)
    names << QString::fromLatin1(kQuality[i].name);
  return names;
}
...
void SynthController::setChordSlot(int slot, int transpose,
                                   const QList<int>& intervals) {
  if (slot < 0 || slot >= SynthProto::CHORD_USER_SLOTS) return;
  ChordUserSlot u;
  u.transpose = qint8(qBound(-24, transpose, 24));
  u.count = quint8(qMin(intervals.size(), qsizetype(SynthProto::CHORD_USER_IVS)));
  for (int i = 0; i < int(u.count); ++i) {
    u.iv[i] = qint8(qBound(-24, intervals.at(i), 36));
  }
  send(OP_CHORD_SET, payloadChordSet(slot, u), true);
}

// An empty slot is silent, and silent is a setting rather than a gap: it is
// how a five-chord set stays quiet on the seven keys it does not use. The
// transpose is kept, so turning a slot off and on again does not lose it.
void SynthController::clearChordSlot(int slot) {
  if (slot < 0 || slot >= m_chordSet.size()) return;
  setChordSlot(slot, m_chordSet.at(slot).transpose, QList<int>());
}
...
void SynthController::setChordSlotQuality(int slot, int quality, int transpose) {
  ...
  for (int i = 0; i < q.count && i < SynthProto::CHORD_USER_IVS; ++i) {
    intervals.append(q.iv[i]);
  }
  setChordSlot(slot, transpose, intervals);
}
...
static QString chordSlotLabel(const QList<int>& intervals) {
  ...
  if (q >= 0) return QString::fromLatin1(kQuality[q].name);
  QStringList parts;
  parts.reserve(count);
...
void SynthController::handleChordSet(const QByteArray& payload) {
  Reader r(payload);
  // Not `slots`: Qt keeps that as a macro (qobjectdefs.h) unless the build
  // defines QT_NO_KEYWORDS, so the declaration would expand to `const int =`.
...
// A fixed-width NUL-padded name field, which is how every string on this link
// travels.
